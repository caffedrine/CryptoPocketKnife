...
void AdvancedTableWidget::ExportAsCSV(QString outputFile, QList<int> columnsToBeExported)
{
    ...
    for( int i = 0; i < this->rowCount(); i++)
    {
        ...
        {
            line += Base::Utils::Csv::EncodeForCSV( this->item(i, columnsToBeExported.at(j))->text()) + ",";
        }
        line += Base::Utils::Csv::EncodeForCSV( this->item(i, columnsToBeExported.at(columnsToBeExported.last()))->text() );
        line += "\n";
