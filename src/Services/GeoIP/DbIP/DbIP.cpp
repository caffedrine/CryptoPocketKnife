...
DbIP::~DbIP()
{
    if (this->countryLookupDb)
    {
        delete this->countryLookupDb;
        this->countryLookupDb = nullptr;
    }
}
...
{
    QString error;
    CountryLookup *countryLookupDb = nullptr;
    if( !countryLookupDb )
    {
        countryLookupDb = DbIP::CountryLookup::load(CountryDbPath, error);
        if( !countryLookupDb )
        {
            qDebug() << "Cannot load database: "<< error;
            return "";
        }
    }
    return countryLookupDb->lookup( QHostAddress(ip_address) );
}
...
struct DbIP::DataFieldDescriptor
{
...
DbIP::DbIP::CountryLookup::CountryLookup(const quint32 size)
        : m_ipVersion(0), m_recordSize(0), m_nodeCount(0), m_nodeSize(0), m_indexSize(0), m_recordBytes(0), m_size(size), m_data(new uchar[size])
{
}

DbIP::CountryLookup *DbIP::DbIP::CountryLookup::load(const QString &filename, QString &error)
{
    CountryLookup *db = nullptr;
    QFile file(filename);
    ...
    db = new CountryLookup(file.size());
...
DbIP::CountryLookup *DbIP::DbIP::CountryLookup::load(const QByteArray &data, QString &error)
{
    CountryLookup *db = nullptr;
    ...
    db = new CountryLookup(data.size());
...
DbIP::DbIP::CountryLookup::~CountryLookup()
{
    delete[] m_data;
}

QString DbIP::CountryLookup::type() const
{
    return m_dbType;
}

quint16 DbIP::CountryLookup::ipVersion() const
{
    return m_ipVersion;
}

QDateTime DbIP::CountryLookup::buildEpoch() const
{
    return m_buildEpoch;
}

QString DbIP::CountryLookup::lookup(const QHostAddress &hostAddr) const
{
...
bool DbIP::CountryLookup::parseMetadata(const QVariantHash &metadata, QString &error)
{
...
bool DbIP::CountryLookup::loadDB(QString &error) const
{
...
QVariantHash DbIP::CountryLookup::readMetadata() const
{
...
QVariant DbIP::CountryLookup::readDataField(quint32 &offset) const
{
...
bool DbIP::CountryLookup::readDataFieldDescriptor(quint32 &offset, DataFieldDescriptor &out) const
{
...
void DbIP::CountryLookup::fromBigEndian(uchar *buf, const quint32 len) const
{
...
QVariant DbIP::CountryLookup::readMapValue(quint32 &offset, const quint32 count) const
{
    QVariantHash map;
...
QVariant DbIP::CountryLookup::readArrayValue(quint32 &offset, const quint32 count) const
{
    QVariantList array;
