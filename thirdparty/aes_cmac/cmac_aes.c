...
static eslt_WorkSpaceCMACAES workspace;
static eslt_WorkSpaceCMACAES workspace_aes256;

uint16_t cmac_aes128_init(uint8_t key[CMAC_AES128_KEY_SIZE])
{
    eslt_ErrorCode retVal = esl_initWorkSpaceHeader(&workspace.header, ESL_MAXSIZEOF_WS_CMACAES, NULL_PTR);

    if( retVal == ESL_ERC_NO_ERROR )
    {
        retVal = esl_initCMACAES128(&workspace, CMAC_AES128_KEY_SIZE, key);
    }
    return retVal;
}
...
{
    return esl_updateCMACAES128(&workspace, length, data);
}

uint16_t cmac_aes128_finish(uint8_t mac[CMAC_AES_MAC_SIZE])
{
    return esl_finalizeCMACAES128(&workspace, mac);
}

uint16_t cmac_aes128_verifyMac(uint8_t inputMac[CMAC_AES_MAC_SIZE])
{
    return esl_verifyCMACAES128(&workspace, inputMac);
}
