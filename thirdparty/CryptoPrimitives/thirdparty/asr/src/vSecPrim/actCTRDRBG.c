 ...
 *                                           continue to copy values to outputString when the function is called the
 *                                           next time
 *  \param[in]     numberOfBytesToReturn Number of bytes that shall be returned by the block cipher DF; this value
 ...
 *  \param[in]      watchdog      watchdog function pointer
 *  \pre            workSpace has to be a valid pointer to a variable of type actCTRDRBGSTRUCT.
 *                  inputData has to be a valid pointer. The buffer referenced by inputData has to provide at least
...
{
  /* ----- Local Variables ------------------------------------------------ */
  actLengthType j, k, startIndexLocal, nextIndexLocal;
  ...
  /* Upper boundary checks for elements is performed at actAesCtrDrbgGenerateSetInput(),
   * esl_initialSeed_CTRDRBG() and esl_reseed_CTRDRBG() */
  for (i = *nextElement; (i < inputString->elements); i++) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER_PARAMETER */
  {
    /* Length for L and N is defined by specification as 4 bytes.
       Length for input_string is defined by specification as 2^35. As this is greater than UINT32_MAX,
       an upper boundary check is not reasonable. */
    for (k = startIndexLocal; (k < inputString->input[i].length); k++) /* FETA_VSECPRIM_UPPER_BOUNDARY_LARGER_UINT32_MAX, FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER_PARAMETER */
    {
      if (remainingBytesInBuffer > 0u)
      {
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  /* 2. n = len (data)/outlen. */
  /* 4. For i = 1 to n do  */
  for (i = 0u; i < Slength; i += actCTRDRBG_BLOCK_LEN) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER */
  {
    if (i == 0u)
    {
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  {
    /* For each call to this function it holds that 
     * - dataLength <= 1024 (max. requested byte length) < uint32max or
     * - dataLength <= 48 (max seed length) < uint32max.
     * Thus, this is no endless loop. */
    for (i = 0u; i <= (dataLength - actCTRDRBG_BLOCK_LEN); i += actCTRDRBG_BLOCK_LEN) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER_PARAMETER */
    {
      /* 2.1 V = (V + 1) mod 2^128 */
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  /* This function has several callers. In one call path, the upper boundary is ensured by a caller within the call tree.
   * In a second callpath, the upper boundary is ensured by the esLib. To reflect both call paths, both FETA justifications are given. */
  for (i = 0u; i < seedLength; i++) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_ESLIB_PARAMETER, FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER_PARAMETER */
  {
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  totalInputLength = 0u;
  for (j = 0u; j < inputString->elements; j++) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER_PARAMETER */
  {
  ...
  if (retVal == actOK)
  {
    bccInputLength = (actLengthType)((((actAESCTRDRBG_DF_BUFFER_OFFSET + totalInputLength) / actCTRDRBG_BLOCK_LEN) + 1u) * actCTRDRBG_BLOCK_LEN);

    /* 9. While len (tempBuffer) < keylen + outlen, do */
    for (j = 0u; j < seedLength; j += actCTRDRBG_BLOCK_LEN) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER */
    {
    ...
    * is a valid key size value, actAESInitEncryptBlock returns actOK. */
    ...
    /* For each call to this function, seedLength is either 32 or 48. Thus, this is no endless loop. */
    /* 13. while len(tempBuffer) < number_of_bits_to_return, do */
    for (j = 0u; j <= (seedLength - actCTRDRBG_BLOCK_LEN); j += actCTRDRBG_BLOCK_LEN) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER */
    {
  ...
  VSECPRIM_P2VAR_PARA(actU8) outputString)
{
  ...
  if (1u < inputString->elements)
  {
    ...
    for (i = 0u; i < personalStrLength; i++) /* FETA_VSECPRIM_UPPER_BOUNDARY_LARGER_UINT32_MAX */
    {
