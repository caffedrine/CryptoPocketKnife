                                                                   ...
                                                                   VSECPRIM_P2VAR_PARA(eslt_Length) cipherSize, VSECPRIM_P2VAR_PARA(eslt_Byte) cipher)
{
  ...
  else
  {
    ...
    if (ESL_ERC_NO_ERROR == returnValue)
    {
      ...
      if (labelSize > 0u)
      {
        if (actOK != ESL_RSA_OAEP_UPDATE_HASH_FCT(&(workSpace->wsHashUnion.SHA256ws), labelSize, label, (workSpace)->header.watchdog))
        {
      ...
      if (ESL_ERC_NO_ERROR == returnValue)
      {
        ESL_RSA_OAEP_FINALIZE_HASH_FCT(&(workSpace->wsHashUnion.SHA256ws), db, (workSpace)->header.watchdog);
      }
    }
    ...
    if (ESL_ERC_NO_ERROR == returnValue)
    {
      ...
      if (ESL_ERC_NO_ERROR == returnValue)
      {
        ...
        returnValue = esl_generateMaskMGF1(ESL_HA_SHA2_256, &(workSpace->wsHashUnion), workSpace->encodedMessageBuffer.tempVariables,
          ESL_SIZEOF_SHA256_DIGEST, ESL_RSA_OAEP_SIZEOF_HASH, seed, dbLength, maskedDb, workSpace->header.watchdog);
      }
      ...
      if (ESL_ERC_NO_ERROR == returnValue)
      {
        ...
        returnValue = esl_generateMaskMGF1(ESL_HA_SHA2_256, &(workSpace->wsHashUnion), workSpace->encodedMessageBuffer.tempVariables,
          ESL_SIZEOF_SHA256_DIGEST, dbLength, maskedDb, ESL_RSA_OAEP_SIZEOF_HASH, maskedSeed, workSpace->header.watchdog);
      }
