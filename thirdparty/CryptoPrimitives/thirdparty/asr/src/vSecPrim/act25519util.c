 ...
 *   = 7237005577332262213973186563042994240857116359379907606001950938285454250989 (253 bit long)
 */
 ...
 *  \param[in,out]  r  In: Pointer to output buffer; Out: Result of operation.
 *  \param[in]      a  Pointer to dividend.
 *  \param[in]      q  Pointer to constant expression q = ((a * m) / 2^512)
 *  \param[in]      n  Pointer to divisor.
 *  \context    TASK
 *  \reentrant  TRUE
 *  \pre        The buffer referenced by r must provide at least BNDIGITS_256 many actBNDIGITs.
 *              The buffer referenced by a must provide at least 2*BNDIGITS_256 many actBNDIGITs.
 *********************************************************************************************************************/
  ...
  VSECPRIM_P2CONST_PARA(actBNDIGIT) n)
{
  ...
  for (d = 0u; d < BNDIGITS_256; d++)
  {
    ...
    digit = n[d];
    tempBuffer = 0u;
    for (j = 0u; j < ((actU32)BNDIGITS_256 - d); j++) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
    {
   ...
   */
  while (actBNCompare(r, n, BNDIGITS_256, FALSE) >= actCOMPARE_EQUAL) /* SBSW_VSECPRIM_CALL_FUNCTION */ /* FETA_VSECPRIM_EDDSA_BARRETT_REDUCTION */
  {
  ...
  VSECPRIM_P2CONST_PARA(actBNDIGIT) a)
{
   ...
   *    -> this implicitly divides the result by 2^512
   */
  for (d = 0u; d < BNDIGITS_256; d++)
  {
  ...
  for (d = BNDIGITS_256; d < (2u * (BNDIGITS_256)); d++)
  {
    tempBuffer += ((actBNDDIGIT)a76[d]) * 76u;
  ...
  for (d = (2u * BNDIGITS_256); d < (3u * (BNDIGITS_256)); d++)
  {
    tempBuffer += ((actBNDDIGIT)a2[offsetA2]) * 2u;
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  ws->auxVar.tempF[0] = (actBNDIGIT)(ws->auxVar.tempF[0] - 2u); /* SBSW_VSECPRIM_WRITE_ARRAY */
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  ws->auxVar.tempC[BNDIGITS_256 - 1u] |= (actBNDIGIT)(ws->auxVar.tempB[0] << (actBN_BITS_PER_DIGIT - 1u)); /* SBSW_VSECPRIM_WRITE_ARRAY */
...
VSECPRIM_FUNC(void) act25519_mulPoint(
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  /* Initialize the result to (0, 1, 1, 0), the neutral element of the group. */
