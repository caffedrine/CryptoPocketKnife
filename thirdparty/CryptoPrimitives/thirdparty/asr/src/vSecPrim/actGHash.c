   ...
   VSECPRIM_P2CONST_PARA(actU8) X)
{
  ...
  /* loop over X: MSB ... LSB */
  for (i = actGHASH_BLOCK_SIZE; i > 0u; i--)
  {
    /* next byte */
    X_i = X[(i -1u)];
...
/*---------------------------------------------------------------------------*/
   ...
   for (i = 4u; i > 0u; i /= 2u) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE_DECREASE */
   {
      gf128_double(info->M[i], info->M[2u * i]);          /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_VARIABLE_SUBADDRESS */
   }
   /* Compose the rest by adding */
   for (i = 2u; i <= 8u; i = (actU8)(i * 2u)) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
   {
      for (j = 1u; j < i; j++) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
      {
...
/*---------------------------------------------------------------------------*/
   ...
   /* Powers of 2 by shifting */
   for (i = 64; i > 0u; i /= 2u) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE_DECREASE */
   {
      gf128_double(info->M[i], info->M[2u * i]);          /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_VARIABLE_SUBADDRESS */
   }
   /* Compose the rest by adding */
   for (i = 2; i <= 128u; i *= 2u) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
   {
      for (j = 1; j < i; j++) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
      {
         gf128_add(info->M[i+j], info->M[i], info->M[j]); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_VARIABLE_SUBADDRESS */
      }
   }
