  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  blockB0[0] = (actU8)(((actU8)8u * ((info->authFieldSize - (actU8)2u) / (actU8)2u)) + info->lengthFieldSize - (actU8)1u); /* SBSW_VSECPRIM_ARRAY_WRITE_ACCESS_PASSED_BUFFER_FIXED_INDEX */

  if (info->aadLen > 0u)
  {
    blockB0[0] = (actU8)(blockB0[0] + 64u); /* SBSW_VSECPRIM_ARRAY_WRITE_ACCESS_PASSED_BUFFER_FIXED_INDEX */
  }
  ...
  const actLengthType blockLength)
{
  ...
  /* ----- Implementation ------------------------------------------------- */
  /* blockLength is calculated by "blockLength = actAES_BLOCK_SIZE - nonceLength - 1u"
   * nonceLength is ensured to be "7 <= nonceLength <= 13" by esl_initAESCCMLengthCheck() which is  ensured to be called
   * before this function. Therefore, it is checked that a valid upper boundary exists. */
  for (i = blockLength; i > 0u; i--) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER_PARAMETER_DECREASE */
  {
    /* Increment block element and store result */
    ++block[(i - 1u)];  /* SBSW_VSECPRIM_PTR_WRITE_ACCESS_PASSED_BUFFER */
    result = block[(i - 1u)];
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  else
  {
    ...
    else
    {
      ...
      /* process aad */
      for (i = 0u; i < numberAadBlocks; i++) /* FETA_VSECPRIM_UPPER_BOUNDARY_LARGER_UINT32_MAX */
      {
      ...
      info->AESCounter[0] = (actU8)(info->lengthFieldSize - (actU8)1u); /* SBSW_VSECPRIM_PTR_WRITE_ACCESS_PASSED_BUFFER */
      ...
      for (i = 0u; i < numberMsgBlocks; i++) /* FETA_VSECPRIM_UPPER_BOUNDARY_LARGER_UINT32_MAX */
      {
  ...
  else
  {
    ...
    else
    {
      ...
      for (i = 0u; i < numberMsgBlocks; i++) /* FETA_VSECPRIM_UPPER_BOUNDARY_LARGER_UINT32_MAX */
      {
        ...
        for (j = 0; j < actMin(actAES_BLOCK_SIZE, remainingLength); j++) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE_PARAMETER */
        {
...
VSECPRIM_FUNC(actRETURNCODE) actAESCCMFinalizeEncrypt(
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  actRETURNCODE retVal = actEXCEPTION_LENGTH;
  actU8 j;
  ...
  if (remainingLength <= actAES_BLOCK_SIZE)
  {
    ...
    for (j = 0; j < (actU8)remainingLength; j++)   /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
    {
    ...
    actMemClear(info->AESCounter, actAES_BLOCK_SIZE); /* PRQA S 0315 */ /* MD_VSECPRIM_P2VOID_CAST */ /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_FIXED_SIZE */
    info->AESCounter[0] = (actU8)(info->lengthFieldSize - (actU8)1u); /* SBSW_VSECPRIM_ARRAY_WRITE_ACCESS_PASSED_BUFFER_FIXED_INDEX */
    ...
    for (j = 0; j < info->authFieldSize; j++) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_ESLIB_PARAMETER */
    {
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  if (remainingLength <= actAES_BLOCK_SIZE)
  {
    if (remainingLength > 0u)
    {
      ...
      for (j = 0; j < remainingLength; j++) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
      {
