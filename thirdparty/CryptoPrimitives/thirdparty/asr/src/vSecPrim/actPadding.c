  ...
  switch (mode)
  {
    ...
    case actPADDING_PM_PKCS7:
    {
      ...
      else
      {
        ...
        /* Upper boundary of buffer_size is set by caller to a value known at compile time */
        for (index = unpad; index < buffer_size; index++) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER_PARAMETER */
        {
          if (buffer[index] != pad)
          {
            result = actEXCEPTION_PADDING;
