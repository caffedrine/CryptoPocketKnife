...
# define actPRIME_OID_TLV_LENGTH                                      (0x09u)
 ...
 *  \synchronous   TRUE
 *********************************************************************************************************************/
VSECPRIM_LOCAL_INLINE_FUNC(actU8) actMemcmp_ROM(
  VSECPRIM_P2ROMCONST_PARA(actU8) buf1,
  VSECPRIM_P2ROMCONST_PARA(actU8) buf2);
 ...
 *
 */
VSECPRIM_LOCAL_INLINE_FUNC(actU8) actMemcmp_ROM(VSECPRIM_P2ROMCONST_PARA(actU8) buf1, VSECPRIM_P2ROMCONST_PARA(actU8) buf2)
{
  actLengthType i;
  boolean isEqual = TRUE;

  for (i = 0; i < actPRIME_OID_TLV_LENGTH; ++i)
  {
    isEqual = (boolean)(isEqual & ((ACT_ROM_BYT(&buf1[i]) == ACT_ROM_BYT(&buf2[i])) ? (TRUE) : (FALSE)));
  }
  ...
  VSECPRIM_P2VAR_PARA(actLengthType) length)
{
  ...
  {
    actLengthType i, startLength = *length, lengthLocal = *length;

    /* skip leading zeros, adjust length to number of leading zeros */
    for (i = startLength; i > 0u; i--)
    {
      if (input[startIndexPrime] == 0u)
      {
        ++startIndexPrime;
        lengthLocal = lengthLocal - 1u;
      }
      else
      {
        break;
    ...
    *length = lengthLocal; /* SBSW_VSECPRIM_WRITE_POINTER */
  ...
  actLengthType wksp_len)
{
  ...
  {
    max_length = (actLengthType)((actECBDWksp(p_length, n_length) * actBN_BYTES_PER_DIGIT) + sizeof(actECCURVE));
  }
  ...
  VSECPRIM_P2VAR_PARA(actLengthType) startIndexNextDataEntry)
{
  ...
  if (retVal == actOK)
  {
    ...
    isRRZero = actBNIsZero(field->RR, digitLengthRR); /* SBSW_VSECPRIM_FCT_CALL_P2CONST_PARAM */

    /* parse p_bar / n_bar */
    if ((isRRZero == TRUE) || (ACT_ROM_BYT(&domain[currentIndex]) != actTAG_OCTETSTRING))
    {
      retVal = actEXCEPTION_DOMAIN_EXT;
    }
    else
  ...
  VSECPRIM_P2VAR_PARA(actLengthType) startIndexData)
{
  ...
  *lengthOfASN1Struct += startIndexDataLocal; /* SBSW_VSECPRIM_WRITE_POINTER */
  ...
  if (retVal == actOK)
  {
    ...
    if (actMemcmp_ROM(&domain[currentIndex], PRIME_OID_TLV) == FALSE) /* PRQA S 0315 */ /* MD_VSECPRIM_P2VOID_CAST */ /* SBSW_VSECPRIM_FCT_CALL_P2CONST_PARAM */
    {
      retVal = actEXCEPTION_DOMAIN;
    }
    else
    {
      currentIndex += (actLengthType)(sizeof(PRIME_OID_TLV));
  ...
  actLengthType p_bytes)
{
  ...
  /* parse points */
  numOfPrecPoints = (actLengthType)(1UL << Curve->groups);
  for (i = 0u; i < numOfPrecPoints; ++i) /* FETA_VSECPRIM_DEFAULT_FOR_LOOP */
  {
    if (retVal == actOK)
    {
  ...
  VSECPRIM_P2VAR_PARA(VSECPRIM_P2CONST_PARA(actU8)) n_addr) /* PRQA S 3673 */ /* MD_VSECPRIM_CONST_POINTER */
{
  ...
  *n_bytes = 0u; /* SBSW_VSECPRIM_PTR_WRITE_ACCESS_PASSED_BUFFER */
  ...
  if (retVal == actOK) /* startIndexData < lengthOfASN1Struct was already checked in actParseSkipUntilFieldPrime */
  {
    ...
    if ((*p_bytes > actECC_MAX_BYTE_LEN) || (*p_bytes == 0u))
    {
      retVal = actEXCEPTION_DOMAIN;
    }
  }
  ...
  if ((retVal == actOK) && ((currentIndex + startIndexNextDataEntry) < lengthOfASN1Struct))
  {
    ...
    if ((*n_bytes > actECC_MAX_BYTE_LEN) || (*n_bytes == 0u))
    {
      retVal = actEXCEPTION_DOMAIN;
    }
  }
...
VSECPRIM_FUNC(actRETURNCODE) actECInit(
  ...
  actLengthType wksp_len)
{
  actRETURNCODE result, resultPEqualsThree, resultNEqualsThree;
  ...
  VSECPRIM_P2VAR_PARA(actBNDIGIT) p_length_3;
  VSECPRIM_P2VAR_PARA(actBNDIGIT) n_length_3;
  ...
  if (result == actOK)
  {
    ...
    if (result == actOK)
    {
      ...
      /* Check p > 3, n > 3 */
      p_length_3 = Curve->t[0];
      actBNSetZero(p_length_3, p_length); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_FIXED_SIZE */
      p_length_3[0] = 3; /* SBSW_VSECPRIM_ARRAY_WRITE_ACCESS_PASSED_BUFFER_FIXED_INDEX */

      n_length_3 = Curve->t[2];
      actBNSetZero(n_length_3, n_length); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_FIXED_SIZE */
      n_length_3[0] = 3; /* SBSW_VSECPRIM_ARRAY_WRITE_ACCESS_PASSED_BUFFER_FIXED_INDEX */

      resultPEqualsThree = actBNCompare(p_field->m, p_length_3, p_length, FALSE); /* SBSW_VSECPRIM_FCT_CALL_P2CONST_PARAM */
      resultNEqualsThree = actBNCompare(n_field->m, n_length_3, n_length, FALSE); /* SBSW_VSECPRIM_FCT_CALL_P2CONST_PARAM */

      if ((resultPEqualsThree <= actCOMPARE_EQUAL) || (resultNEqualsThree <= actCOMPARE_EQUAL))
      {
        result = actEXCEPTION_DOMAIN;
      }
      else
      {
        /* check if a = p - 3 */
        ...
        p_minus_3 = p_length_3; /* reuse memory */
        actBNSub(p_field->m, p_length_3, p_minus_3, p_length); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_FIXED_SIZE */
        if (actBNCompare(a, p_minus_3, p_length, FALSE) == actCOMPARE_EQUAL) /* SBSW_VSECPRIM_FCT_CALL_P2CONST_PARAM */
        {
