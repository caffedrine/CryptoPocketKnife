  ...
  VSECPRIM_P2VAR_PARA(eslt_Byte) key)
{
  ...
  else
  {
    ...
    {
      /* derive key with actCLib HKDF HMAC SHA-256 one-step method */
      actRetVal = actHKDF_HMAC_OneStepDerive(&workSpace->wsHKDF, secret, secretLength, salt, saltLength, info, infoLength, key, keyLength, actHASHMACSHA256_ALGO_ID, workSpace->header.watchdog); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
    }
    else
    {
      /* derive key with actCLib HKDF HMAC SHA-256 two-step method */
      actHKDF_HMAC_Extract(&workSpace->wsHKDF, secret, (actLengthType)secretLength, salt, (actLengthType)saltLength, actHASHMACSHA256_ALGO_ID, workSpace->header.watchdog); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
      actRetVal = actHKDF_HMAC_Expand(&workSpace->wsHKDF, info, (actLengthType)infoLength, key, (actLengthType)keyLength, actHASHMACSHA256_ALGO_ID, workSpace->header.watchdog); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
    }
