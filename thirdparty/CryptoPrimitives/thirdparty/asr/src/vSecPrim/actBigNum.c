...
# elif (2 == actBN_BYTES_PER_DIGIT) /* COV_VSECPRIM_NO_SAFE_CONFIG TX */
    tmp = ((actBNDIGIT)(bytes[offset]));
    tmp = (actBNDIGIT)(tmp + (((actBNDIGIT)(bytes[offset + 1u])) << 8));
    offset += 2u;
  ...
  const actLengthType length)
{
  ...
  for (i = 0u; i < length; ++i) /* FETA_VSECPRIM_BIGNUM_BOUND_LIMITED_BY_PREVIOUSLY_CHECKED_PARAMETER */
  {
  ...
  const actLengthType bit_pos)
{
  ...
  {
    a[bit_pos / actBN_BITS_PER_DIGIT] |= (actBNDIGIT)(((actBNDIGIT) 1) << (bit_pos % actBN_BITS_PER_DIGIT)); /* SBSW_VSECPRIM_WRITE_ARRAY */
    retVal = actOK;
  }
  ...
  const actBNDIGIT  value)
{
   ...
   for (i = 1u; i < length; ++i) /* FETA_VSECPRIM_BIGNUM_BOUND_LIMITED_BY_PREVIOUSLY_CHECKED_PARAMETER */
   {
  ...
  const boolean completeRuntime)
{
  ...
  for (i = 0u; i < length; i++) /* PRQA S 0771 */ /* MD_VSECPRIM_14.6 */ /* FETA_VSECPRIM_BIGNUM_BOUND_LIMITED_BY_PREVIOUSLY_CHECKED_PARAMETER */
  {
  ...
  /* From here on, the return value is known and not supposed to be changed.
   * We complete iterating over the buffers anyway, so that the runtime of this
   * function is independent from the result of the comparison. */

  if (completeRuntime == TRUE)
  {
    /* If the previous for loop is stopped with 'break', i is not yet incremented. 
     * Since the second loop is only executed for secret/private data, all input parameter 
     * lengths are compared to configured values < uint32max before this call.
     * Thus, no overflow occurs. */
    i = i + 1u;
    for (; i < length; i++) /* FETA_VSECPRIM_BIGNUM_BOUND_LIMITED_BY_PREVIOUSLY_CHECKED_PARAMETER */
    {
      if (a[((length - 1u) - i)] < b[((length - 1u) - i)])
      {
        /* do nothing */
        VSECPRIM_DUMMY_STATEMENT(retVal);
      }
      else if (a[((length - 1u) - i)] > b[((length - 1u) - i)])
      {
        /* do nothing */
  ...
  const actLengthType in_len)
{
  ...
  for (i = 0u; i < digits; ++i) /* FETA_VSECPRIM_BIGNUM_BOUND_LIMITED_BY_PREVIOUSLY_CHECKED_PARAMETER */
  {
...
# if (actBN_BYTES_PER_DIGIT != 1) /* COV_VSECPRIM_NO_SAFE_CONFIG TX */
  if (in_len <= ((actLengthType)length * actBN_BYTES_PER_DIGIT))
  {
    ...
    {
      a[digits] = (actBNDIGIT)(a[digits] << 8u); /* SBSW_VSECPRIM_WRITE_ARRAY */
      a[digits] = (actBNDIGIT)(a[digits] | (actBNDIGIT)(in[((offset - i) + 1u)])); /* SBSW_VSECPRIM_WRITE_ARRAY */
    }
  }
# endif
}
  ...
  const actLengthType in_len)
{
  ...
  /* copy bytes to digits */
  for (i = 0; i < digits; i++) /* FETA_VSECPRIM_BIGNUM_BOUND_LIMITED_BY_PREVIOUSLY_CHECKED_PARAMETER */
  {
# if (actBN_BYTES_PER_DIGIT == 1) /* COV_VSECPRIM_NO_SAFE_CONFIG XF */
...
# if (actBN_BYTES_PER_DIGIT != 1) /* COV_VSECPRIM_NO_SAFE_CONFIG TX */
  if (in_len <= (actLengthType)(length * actBN_BYTES_PER_DIGIT))
  {
    ...
    {
      a[digits] = (actBNDIGIT)(a[digits] << 8u); /* SBSW_VSECPRIM_WRITE_ARRAY */
      a[digits] = (actBNDIGIT)(a[digits] | (actBNDIGIT)(ACT_ROM_BYT(&in[((offset - i) + 1u)]))); /* SBSW_VSECPRIM_WRITE_ARRAY */
    }
  }
# endif
}
