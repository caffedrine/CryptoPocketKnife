  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
   ...
   else
   {
      ...
      if (actBNCompare(tmp1, tmp2, p_length, FALSE) != actCOMPARE_EQUAL) /* SBSW_VSECPRIM_FCT_CALL_P2CONST_PARAM */
      {
         retVal = actEXCEPTION_POINT;
      }
   }
...
{
  actLengthType i;
  const actLengthType orderBitLength = actBNGetBitLength(Curve->n_field.m, Curve->n_field.m_length); /* SBSW_VSECPRIM_FCT_CALL_P2CONST_PARAM */
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  /* Double-And-Add loop */
  while (mask != 0u) /* FETA_VSECPRIM_WHILE_LOOP */
  {
    /* R *= 2 */
    actECPDouble(R, Curve, watchdog); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
   ...
   actLengthType prec_offset = 3u;

   actLengthType prec_length;
...
# if (vSecPrim_GetECPMaxKeySizeOfGeneral() >= VSECPRIM_ECP_MAX_KEY_SIZE_512) /* COV_VSECPRIM_NO_SAFE_CONFIG XF */
   if (p_bytes >= 64u)
   {
     prec_offset = 4u;
   }
# endif
   prec_length = prec_offset + (2u * p_bytes);

   /* calculate group_bits */
   for (i = n_bits; (i % Curve->groups) != 0u; ++i) /* FETA_VSECPRIM_ADD_UNTIL_REMAINDER_ZERO */
   {
     /* Do nothing; the main task of this loop is to increment i.
      * At most Curve->groups many iterations are required. */
   }
