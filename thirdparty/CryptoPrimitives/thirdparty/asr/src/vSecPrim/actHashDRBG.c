  ...
  VSECPRIM_P2VAR_PARA(actU8) result)
{
  ...
  /* ----- Implementation ------------------------------------------------- */
  /* bufferBLength is either
   * - actHASH_SIZE_SHA512 = 64 < uint32max or
   * - actHASHDRBG_SHA512_SEED_LEN = 111 < uint32max or
   * - actDRBG_SIZEOF_INPUTLENGTH = 4 < uint32max.
   * Thus, this is no endless loop. */
  for (i = 1; i <= bufferBLength; i++)
  {
    tmp = (actU16)(((actU16)bufferA[actHASHDRBG_SHA512_SEED_LEN - i]) + bufferB[bufferBLength - i] + actGetU8fromU16(tmp, 2u));
    result[actHASHDRBG_SHA512_SEED_LEN - i] = (actU8)tmp; /* SBSW_VSECPRIM_WRITE_ARRAY */
  }

  /* Since actHASHDRBG_SHA512_SEED_LEN = 111 < uint32max, this is no endless loop. */
  for (i = bufferBLength + 1u; i <= actHASHDRBG_SHA512_SEED_LEN; i++)
  {
    tmp = (actU16)(((actU16)bufferA[actHASHDRBG_SHA512_SEED_LEN - i]) + actGetU8fromU16(tmp, 2u));
    result[actHASHDRBG_SHA512_SEED_LEN - i] = (actU8)tmp; /* SBSW_VSECPRIM_WRITE_ARRAY */
  }
}
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  /* Fixed value for len is set by caller and known at compile time. */
  for (j = 0u; j < len; j++) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER_PARAMETER */
  {
    ...
    /* This function has several callers from act and esl functions. In the act-callpath, the upper boundary is
       ensured by a caller within the call tree. In the esl-callpath, the upper boundary is ensured by the
       esLib. To reflect both call paths, both FETA justifications are provided. */
    for (i = 0u; i < inputString->elements; i++) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_ESLIB_PARAMETER, FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER_PARAMETER */
    {
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  if (m > 0u)
  {
    ...
    for (i = 0u; i < (m - 1u); i++) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_ESLIB_PARAMETER */
    {
      (void)actSHA512Init(&workSpace->wsSHA512); /* SBSW_VSECPRIM_CALL_FUNCTION */
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  for (j = 0u; j < hashInputString.elements; j++) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
  {
    /* actSHA512Update returns actEXCEPTION_LENGTH, if input length is greater than the maximal
