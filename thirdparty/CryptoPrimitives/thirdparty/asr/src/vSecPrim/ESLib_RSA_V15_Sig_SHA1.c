...
# define ESL_RSA_V15_INIT_HASH_FCT(workspace)                         actSHAInit(workspace)
...
# define ESL_RSA_V15_FINALIZE_HASH_FCT(workspace, messageDigest, wd)  actSHAFinalize(workspace, (VSECPRIM_P2VAR_PARA(actU8))(messageDigest), wd)
                                                          ...
                                                          VSECPRIM_P2VAR_PARA(eslt_Length) signatureSize, VSECPRIM_P2VAR_PARA(eslt_Byte) signature)
{
  ...
  else
  {
    ...
    else
    {
      ...
      i = (eslt_Length)(i + sizeof(eslt_SIGN_RSA_SHA1_V15_ASN1_DIGESTINFO));
      /* actSHAFinalize always returns actOK */
      (void)ESL_RSA_V15_FINALIZE_HASH_FCT(&(workSpace->wsHashUnion.SHA1ws), &messageBuf[i], (workSpace)->header.watchdog);
