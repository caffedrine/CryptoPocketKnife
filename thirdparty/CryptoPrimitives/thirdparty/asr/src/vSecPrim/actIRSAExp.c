  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  if (key_flag != actRSA_PRIVATE_KEY_OPERATION_CRT) /* checks are done in actRSAInitPrivateKeyOperationCRT */
  {
    ...
    if (returnValue == actOK)
    {
      if (actBNCompare(e, modulus, n_length, TRUE) >= actCOMPARE_EQUAL) /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_FIXED_SIZE */
      {
        returnValue = key_out_of_range;
      }
    }
...
# endif
    ...
    if ( returnValue != actOK)
    {
      /* n_ring->m = 0, i.e., modulus too small */
      returnValue = actRSA_PARAM_OUT_OF_RANGE;
    }
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  if (returnValue == actOK)
  {
    ...
    /* check 0 <= c < modulus */
    if (actBNCompare(c, n_ring->m, n_length, FALSE) >= actCOMPARE_EQUAL) /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_FIXED_SIZE */
    {
      returnValue = actRSA_PARAM_OUT_OF_RANGE;
    }
