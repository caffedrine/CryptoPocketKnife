                                                       ...
                                                       const eslt_Length infoLength, VSECPRIM_P2CONST_PARA(eslt_Byte) info, const eslt_Length keyLength, VSECPRIM_P2VAR_PARA(eslt_Byte) key)
{
  ...
  else
  {
     ...
     actRETURNCODE retValAct = actKDFX963Derive(&workSpace->wsKDFX963SHA1, secret, secretLength, info, infoLength, key, keyLength, workSpace->header.watchdog);  /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
