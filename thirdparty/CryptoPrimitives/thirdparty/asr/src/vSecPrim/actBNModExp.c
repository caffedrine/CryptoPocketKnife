...
# if (actBN_MOD_EXP_WINDOW_SIZE==1) /* COV_VSECPRIM_NO_SAFE_CONFIG TX */
  ...
  const actLengthType modulusBitLength = actBNGetBitLength(Ring->m, Ring->m_length) - 1u; /* SBSW_VSECPRIM_FCT_CALL_P2CONST_PARAM */
  ...
  /* set j to the index of the most significant bit of the exponent 'e' */
  for (j = modulusBitLength; j > 0u; j--)
  {
    if (actBNGetBit(e, j) == FALSE)
    {
      if ((j & 0x07u) == 0x00u)
      {
