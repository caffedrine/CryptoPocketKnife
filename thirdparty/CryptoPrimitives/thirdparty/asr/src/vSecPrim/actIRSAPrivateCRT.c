  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  else
  {
    ...
    else if (actBNCompare(qInv, p, p_length, TRUE) >= actCOMPARE_EQUAL)
    {
      retVal = actEXCEPTION_PRIVKEY;
    }
    else
    {
      ...
      else if (actBNCompare(dp, p, p_length, TRUE) >= actCOMPARE_EQUAL)
      {
        retVal = actEXCEPTION_PRIVKEY;
      }
      ...
      else if (actBNCompare(dq, q, q_length, TRUE) >= actCOMPARE_EQUAL)
      {
        retVal = actEXCEPTION_PRIVKEY;
      }
      else
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  if (actBNCompare(origCipherBeforeReduction, fullModulus, n_length, FALSE) >= actCOMPARE_EQUAL)
  {
    return actRSA_PARAM_OUT_OF_RANGE;
  }
