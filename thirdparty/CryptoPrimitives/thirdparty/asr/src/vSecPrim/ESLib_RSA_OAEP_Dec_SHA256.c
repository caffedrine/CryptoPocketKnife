...
# define ESL_RSA_OAEP_INIT_HASH_FCT(workspace)                        (void)actSHA256Init(workspace)
/* Hash update function */
# define ESL_RSA_OAEP_UPDATE_HASH_FCT(workspace, inputSize, input, wd) actSHA256Update(workspace, (VSECPRIM_P2CONST_PARA(actU8))(input), (actLengthType)(inputSize), wd)
/* Hash finalization function */
# define ESL_RSA_OAEP_FINALIZE_HASH_FCT(workspace, messageDigest, wd) (void)actSHA256Finalize(workspace, (VSECPRIM_P2VAR_PARA(actU8))(messageDigest), wd)
                                                       ...
                                                       VSECPRIM_P2VAR_PARA(eslt_Length) messageSize, VSECPRIM_P2VAR_PARA(eslt_Byte) message)
{
  ...
  else
  {
    ...
    if (ESL_ERC_NO_ERROR == returnValue)
    {
      ...
      returnValue = esl_generateMaskMGF1(ESL_HA_SHA2_256, workSpace->wsHashUnionPtr, workSpace->encodedMessageBufferPtr->tempVariables,
        ESL_SIZEOF_SHA256_DIGEST, dbLength, maskedDb, ESL_RSA_OAEP_SIZEOF_HASH, db, workSpace->header.watchdog);
      ...
      if (ESL_ERC_NO_ERROR == returnValue)
      {
        ...
        returnValue = esl_generateMaskMGF1(ESL_HA_SHA2_256, workSpace->wsHashUnionPtr, workSpace->encodedMessageBufferPtr->tempVariables,
          ESL_SIZEOF_SHA256_DIGEST, ESL_RSA_OAEP_SIZEOF_HASH, maskedSeed, dbLength, db, workSpace->header.watchdog);
      }
      ...
      if (ESL_ERC_NO_ERROR == returnValue)
      {
        ...
        if (labelSize > 0u)
        {
          if (actOK != ESL_RSA_OAEP_UPDATE_HASH_FCT(&(workSpace->wsHashUnionPtr->SHA256ws), labelSize, label, (workSpace)->header.watchdog))
          {
        ...
        if (ESL_ERC_NO_ERROR == returnValue)
        {
          ESL_RSA_OAEP_FINALIZE_HASH_FCT(&(workSpace->wsHashUnionPtr->SHA256ws), maskedSeed, (workSpace)->header.watchdog);
        }
      }
