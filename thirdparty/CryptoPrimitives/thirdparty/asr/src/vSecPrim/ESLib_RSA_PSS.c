...
VSECPRIM_P2VAR_PARA(eslt_Byte) saltedHash)
{
  ...
  else
  {
    ...
    wsHashUnion.SHA1ws = wsHash->wsSHA1;

    returnValue = esl_calcSaltedHashRSA_PSS(&wsHashUnion, saltSize,  /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
      salt, messageDigest, ESL_SIZEOF_SHA1_DIGEST, saltedHash, ESL_HA_SHA1, wsHash->header.watchdog);
  ...
  VSECPRIM_P2VAR_PARA(eslt_Byte) saltedHash)
{
  ...
  {
    eslt_WorkSpaceUnionHashFunctions_RSA wsHashUnion; /* PRQA S 0759 */ /* MD_MSR_Union */
    wsHashUnion.SHA256ws = wsHash->wsSHA256;

    returnValue = esl_calcSaltedHashRSA_PSS(&wsHashUnion, saltSize, /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_RSAPSSSHA256_SALTEDHASH */
      salt, messageDigest, ESL_SIZEOF_SHA256_DIGEST, saltedHash, ESL_HA_SHA2_256, wsHash->header.watchdog);
...
VSECPRIM_P2VAR_PARA(eslt_Byte) saltedHash)
{
  ...
  {
    eslt_WorkSpaceUnionHashFunctions_RSA wsHashUnion; /* PRQA S 0759 */ /* MD_MSR_Union */
    wsHashUnion.RIPEMD160ws = wsHash->wsRIPEMD160;

    returnValue = esl_calcSaltedHashRSA_PSS(&wsHashUnion, saltSize, /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
      salt, messageDigest, ESL_SIZEOF_RIPEMD160_DIGEST, saltedHash, ESL_HA_RMD160, wsHash->header.watchdog);
