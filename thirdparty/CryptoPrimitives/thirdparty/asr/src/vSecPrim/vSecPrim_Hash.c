...
/* table of digest sizes in byte of all provided hash algorithms */
...
# if (VSECPRIM_SHA3_ENABLED == STD_ON)
  ...
  case VSECPRIM_DIGEST_SHA3_512:
     ...
     * not sufficient. This is not the case, so it is safe to ignore the return value. */
    (void)esl_initSHA3(&(workSpace->wsHash.wsSHA3), (eslt_Length)(Esl_BytesToBits((eslt_Length)vSecPrim_digestSizes[hashID]))); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
    break;
  ...
  case VSECPRIM_DIGEST_SHAKE_256:
     ...
     * not sufficient. This is not the case, so it is safe to ignore the return value. */
    (void)esl_initSHAKE(&(workSpace->wsHash.wsSHAKE), (eslt_Length)(Esl_BytesToBits(Esl_DivideByTwo((eslt_Length)(vSecPrim_digestSizes[hashID]))))); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
    break;
# endif
  ...
  vSecPrimType_OutputBufferFlag outputLengthFlag)
{
  ...
  else if ((retVal = esl_CheckWorkSpaceHeader(&workSpace->header, VSECPRIM_SIZEOF_WS_HASH)) != ESL_ERC_NO_ERROR) /* PRQA S 3326 */ /* MD_VSECPRIM_13.1 */ /* SBSW_VSECPRIM_CALL_FUNCTION */
  {
    /* retVal already set */
  }
