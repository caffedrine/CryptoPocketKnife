  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  while ((rest > 0u) || (processLastBlock == TRUE)) /* FETA_VSECPRIM_ACTCMACAES_WHILE_LOOP */
  {
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  for (keyIndex = 0u; keyIndex < keyCount; keyIndex++) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
  {
    msb = subKey[0] & CMAC_MSB;

    for (index = 0; index < (actAES_BLOCK_SIZE - 1u); index++)
    {
      subKey[index] = (actU8) (((actU8)(subKey[index] << 1u) | (actU8)(subKey[index + 1u] >> 7u))); /* SBSW_VSECPRIM_WRITE_ARRAY_NO_CHECK */
    }

    subKey[index] = (actU8) ((actU8)(subKey[index] << 1u)); /* SBSW_VSECPRIM_WRITE_ARRAY_NO_CHECK */
