                                                                 ...
                                                                 VSECPRIM_P2VAR_PARA(eslt_Length) cipherSize, VSECPRIM_P2VAR_PARA(eslt_Byte) cipher)
{
  ...
  else if ((returnValue = esl_CheckWorkSpaceHeaderAndState(&workSpace->header, ESL_MAXSIZEOF_WS_RSA_OAEP_ENC, ESL_WST_ALGO_RSA)) != ESL_ERC_NO_ERROR) /* PRQA S 3326 */ /* MD_VSECPRIM_13.1 */
  {
  ...
  else
  {
    ...
    if (ESL_ERC_NO_ERROR == returnValue)
    {
      ...
      if (labelSize > 0u)
      {
        if (actOK != ESL_RSA_OAEP_UPDATE_HASH_FCT(&(workSpace->wsHashUnion.SHA1ws), labelSize, label, (workSpace)->header.watchdog))
        {
      ...
      if (ESL_ERC_NO_ERROR == returnValue)
      {
        ESL_RSA_OAEP_FINALIZE_HASH_FCT(&(workSpace->wsHashUnion.SHA1ws), db, (workSpace)->header.watchdog);
      }
    }
    ...
    if (ESL_ERC_NO_ERROR == returnValue)
    {
      ...
      dbLength = (eslt_Length)(keyPairModuleSize - (ESL_RSA_OAEP_SIZEOF_HASH + 1u));
      /* Points to maskedDb in EM */
      maskedDb = &encodedMessage[ESL_RSA_OAEP_SIZEOF_HASH + 1u];

      /* Calculate length of PS in DB */
      psLength = (eslt_Length)(dbLength - (messageSize + ESL_RSA_OAEP_SIZEOF_HASH + 1u));
      /* Points to PS in DB */
      ...
      if (ESL_ERC_NO_ERROR == returnValue)
      {
        ...
        returnValue = esl_generateMaskMGF1(ESL_HA_SHA1, &(workSpace->wsHashUnion), workSpace->encodedMessageBuffer.tempVariables,
          ESL_SIZEOF_SHA1_DIGEST, ESL_RSA_OAEP_SIZEOF_HASH, seed, dbLength, maskedDb, workSpace->header.watchdog);
      }
      ...
      if (ESL_ERC_NO_ERROR == returnValue)
      {
        ...
        returnValue = esl_generateMaskMGF1(ESL_HA_SHA1, &(workSpace->wsHashUnion), workSpace->encodedMessageBuffer.tempVariables,
          ESL_SIZEOF_SHA1_DIGEST, dbLength, maskedDb, ESL_RSA_OAEP_SIZEOF_HASH, maskedSeed, workSpace->header.watchdog);
      }
...
/**********************************************************************************************************************
 *  END OF FILE: ESLib_RSA_OAEP_Enc_SHA1.c
 *********************************************************************************************************************/
