...
# define ESL_RSA_OAEP_INIT_HASH_FCT(workspace)                        (void)actSHAInit(workspace)
/* Hash update function */
# define ESL_RSA_OAEP_UPDATE_HASH_FCT(workspace, inputSize, input, wd) actSHAUpdate(workspace, (VSECPRIM_P2CONST_PARA(actU8))(input), (actLengthType)(inputSize), wd)
/* Hash finalization function */
# define ESL_RSA_OAEP_FINALIZE_HASH_FCT(workspace, messageDigest, wd) (void)actSHAFinalize(workspace, (VSECPRIM_P2VAR_PARA(actU8))(messageDigest), wd)
...
VSECPRIM_FUNC(eslt_ErrorCode) esl_decodeRSASHA1_OAEP(
                                  VSECPRIM_P2VAR_PARA(eslt_WorkSpace) workSpace, eslt_Length encodedMessageSize, /* PRQA S 3673 */ /* MD_VSECPRIM_16.7 */
                                  ...
                                  VSECPRIM_P2VAR_PARA(eslt_Length) messageSize, VSECPRIM_P2VAR_PARA(eslt_Byte) message)
{
  ...
  returnValue = ESL_ERC_NO_ERROR; /* PRQA S 2982 */ /* MD_MSR_RetVal */

  if ((workSpace == NULL_PTR) || ((label == NULL_PTR) && (labelSize > 0u)) || (messageSize == NULL_PTR) || (message == NULL_PTR))
  {
    returnValue = ESL_ERC_PARAMETER_INVALID;
  }
  else if ((returnValue = esl_CheckWorkSpaceHeaderAndState(&workSpace->header, ESL_SIZEOF_WS_RSA_OAEP_DECODING, ESL_WST_ALGO_RSA)) != ESL_ERC_NO_ERROR) /* PRQA S 3326 */ /* MD_VSECPRIM_13.1 */
  {
  ...
  else
  {
    ...
    if (ESL_ERC_NO_ERROR == returnValue)
    {
      ...
      dbLength = (eslt_Length)(encodedMessageSize - (ESL_RSA_OAEP_SIZEOF_HASH + 1u));
      ...
      returnValue = esl_generateMaskMGF1(ESL_HA_SHA1, workSpace->wsHashUnionPtr, workSpace->encodedMessageBufferPtr->tempVariables,
        ESL_SIZEOF_SHA1_DIGEST, dbLength, maskedDb, ESL_RSA_OAEP_SIZEOF_HASH, db, workSpace->header.watchdog);
      ...
      if (ESL_ERC_NO_ERROR == returnValue)
      {
        ...
        returnValue = esl_generateMaskMGF1(ESL_HA_SHA1, workSpace->wsHashUnionPtr, workSpace->encodedMessageBufferPtr->tempVariables,
          ESL_SIZEOF_SHA1_DIGEST, ESL_RSA_OAEP_SIZEOF_HASH, maskedSeed, dbLength, db, workSpace->header.watchdog);
      }
      ...
      if (ESL_ERC_NO_ERROR == returnValue)
      {
        ...
        if (labelSize > 0u)
        {
          if (actOK != ESL_RSA_OAEP_UPDATE_HASH_FCT(&(workSpace->wsHashUnionPtr->SHA1ws), labelSize, label, (workSpace)->header.watchdog))
          {
        ...
        if (ESL_ERC_NO_ERROR == returnValue)
        {
          ESL_RSA_OAEP_FINALIZE_HASH_FCT(&(workSpace->wsHashUnionPtr->SHA1ws), maskedSeed, (workSpace)->header.watchdog);
        }
      }
      ...
      if (ESL_ERC_NO_ERROR == returnValue)
      {
        ...
        else
        {
          ...
          messageLength = (eslt_Length)(dbLength - (psLength + ESL_RSA_OAEP_SIZEOF_HASH + 1u));
                                  ...
                                  VSECPRIM_P2VAR_PARA(eslt_Length) messageSize, VSECPRIM_P2VAR_PARA(eslt_Byte) message)
{
  ...
  else if ((returnValue = esl_CheckWorkSpaceHeaderAndState(&workSpace->header, ESL_MAXSIZEOF_WS_RSA_OAEP_DEC, ESL_WST_ALGO_RSA)) != ESL_ERC_NO_ERROR) /* PRQA S 3326 */ /* MD_VSECPRIM_13.1 */
  {
    /* return value already set */
  }
  else
  {
    ...
    if (ESL_ERC_NO_ERROR == returnValue)
    {
      ...
      (void)esl_initWorkSpaceHeader(&wsDecoding.header, ESL_SIZEOF_WS_RSA_OAEP_DECODING, workSpace->header.watchdog);
      ...
      esl_SetWorkspaceStatus(&wsDecoding.header, ESL_WST_ALGO_RSA);
