...
{
  /* There is no overflow when adding a_length and b_length, since the function is called with 
   * - either a_length <= BNDIGITS_512, b_length <= BNDIGITS_256 (in Ed25519),
   * - or a_length <= ((max RSA modulus length / 2) + 4), b_length <= ((max RSA modulus length / 2) + 4) (in RSA CRT),
   *   max RSA modulus length <= 4096. */
  ...
  for (i = 0u; i < b_length; ++i) /* FETA_VSECPRIM_BIGNUM_BOUND_LIMITED_BY_PREVIOUSLY_CHECKED_PARAMETER */
  {
    carry = 0u;
    for (j = 0u; j < a_length; ++j) /* FETA_VSECPRIM_BIGNUM_BOUND_LIMITED_BY_PREVIOUSLY_CHECKED_PARAMETER */
    {
