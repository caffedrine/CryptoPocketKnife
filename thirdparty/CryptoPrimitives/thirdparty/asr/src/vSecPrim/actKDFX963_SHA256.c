 ...
 *
 */
VSECPRIM_FUNC(actRETURNCODE) actKDFX963_SHA256_Derive(
...
# if (vSecPrim_GetSizeOfEsltLengthOfGeneral() == VSECPRIM_SIZE_UINT32) /* COV_VSECPRIM_NO_SAFE_CONFIG TX */
  if (((key_length + (actHASH_SIZE_SHA256 - 1u)) < key_length))
  {
...
# endif
  {
    ...
    /* derive key */
    /* Since key_length + actHASH_SIZE_SHA256 - 1u < uint32max < 2^32 it follows that
     * blocks < (2^32)/actHASH_SIZE_SHA256 = (2^32)/32 = (2^32)/2^5 = 2^27 < uint32max.
