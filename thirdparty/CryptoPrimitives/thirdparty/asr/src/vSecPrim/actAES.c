  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog) (void)) /* PRQA S 0850 */ /* MD_MSR_MacroArgumentEmpty */
{
  ...
  /* Upper boundary check for blocks is performed in actAESTransform() */
  for (bcount = 0u; bcount < (blocks*actAES_BLOCK_SIZE); bcount += actAES_BLOCK_SIZE) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLER_PARAMETER */
  {
    /* out_block = prev_block ^ in_block */
    for (i = 0u; i < actAES_BLOCK_SIZE; ++i)
    {
      actLengthType j = bcount + i;
      out_buffer[j] = (actU8)(tempPrevBlock[j - bcount] ^ in[j]); /* SBSW_VSECPRIM_WRITE_ARRAY_NO_CHECK */
    }
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog) (void)) /* PRQA S 0850 */ /* MD_MSR_MacroArgumentEmpty */
{
  ...
  /* Upper boundary check for blocks is performed in actAESTransform() */
  for (bcount = 0u; bcount < (blocks*actAES_BLOCK_SIZE); bcount += actAES_BLOCK_SIZE) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER_PARAMETER */
  {
    ...
    for (i = bcount; i < overallInLength; ++i)  /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE_PARAMETER */
    {
...
VSECPRIM_LOCAL_FUNC(void) actAESCTRIncrementCounter(VSECPRIM_P2VAR_PARA(actU8) counter)
{
  ...
  {
    tmp = (actU16)(((actU16)counter[actAES_BLOCK_SIZE - j]) + one[actAES_BLOCK_SIZE - j] + actGetU8fromU16(tmp, 2u));
  ...
  counter[actAES_BLOCK_SIZE - 1u] = (actU8)(counter[actAES_BLOCK_SIZE - 1u] + actGetU8fromU16(tmp, 2u)); /* SBSW_VSECPRIM_ARRAY_WRITE_ACCESS_PASSED_BUFFER_FIXED_INDEX */
}
...
# if (actAES_SPEED_UP == VSECPRIM_SPEED_UP_LEVEL_0) /* COV_VSECPRIM_NO_SAFE_CONFIG XF */
  ...
  for (n = 0u; n < 4u; ++n)
  {
    ...
    for (i = 0u; i < 4u; ++i)
    {
      t = sbx_tab[actGetByte(bi[(actU8)((n + i) & (actU8)3u)], i)];
...
# if (actAES_SPEED_UP <= VSECPRIM_SPEED_UP_LEVEL_1) /* COV_VSECPRIM_NO_SAFE_CONFIG XF */
  ...
  for (n = 0u; n < 4u; ++n)
  {
    ...
    for (i = 0u; i < 4u; ++i)
    {
      bo[n] ^= (((actU32) ACT_ROM_R_BYT(sbx_tab[actGetByte(bi[(actU8)((n + i) & (actU8)3u)], i)])) << (i << 3u));
    }
  }
...
VSECPRIM_FUNC(actRETURNCODE) actAESKeySetup(VSECPRIM_P2VAR_PARA(actAESSTRUCT) info, VSECPRIM_P2CONST_PARA(actU8) key, actLengthType key_len, VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog) (void)) /* PRQA S 0850 */ /* MD_MSR_MacroArgumentEmpty */
{
  ...
  else
  {
    ...
    for (i = 0u; i < key_dword_len; ++i) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE_PARAMETER */
    {
    ...
    for (i = 0u; (i < (iterations % 11u)); ++i) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE_PARAMETER */
    {
      actL2trigger(watchdog); /* PRQA S 1338, 2983, 3112  */ /* MD_MSR_DummyStmt */ /*lint -e438 */ /* SBSW_VSECPRIM_FUNCTION_CALL_WATCHDOG */

      t = actRotr(t, 8u);
      t = ls_box(t) ^ ACT_ROM_R_DWD(rco_tab[(i % 10u)]);  /* PRQA S 2985 */ /* MD_VSECPRIM_REDUNDANT_OP */
      for (j = 0u; j < key_dword_len; ++j) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE_PARAMETER */
      {
...
VSECPRIM_FUNC(void) actAESMakeDecryptionKey(VSECPRIM_P2VAR_PARA(actAESSTRUCT) info)
{
  ...
  /* Upper boundary check for info->key_dword_len is implemented in actAESKeySetup() */
  for (i = 4u; i < iterations; ++i) /* FETA_VSECPRIM_BOUNDARY_CHECK_BY_CALLER_PARAMETER */
  {
    u = star_x(tempKey[i]);
    v = star_x(u);
    w = star_x(v);
