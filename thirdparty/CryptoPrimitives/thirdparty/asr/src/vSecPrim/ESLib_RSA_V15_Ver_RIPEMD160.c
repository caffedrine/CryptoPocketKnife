...
# define ESL_RSA_V15_INIT_HASH_FCT(workspace)                         actRMD160Init(workspace)
/* Hash update function */
# define ESL_RSA_V15_UPDATE_HASH_FCT(workspace, inputSize, input, wd) actRMD160Update(workspace, (VSECPRIM_P2CONST_PARA(actU8))(input), (inputSize), wd)
/* Hash finalization function */
# define ESL_RSA_V15_FINALIZE_HASH_FCT(workspace, messageDigest, wd)  actRMD160Finalize(workspace, (VSECPRIM_P2VAR_PARA(actU8))(messageDigest), wd)
...
VSECPRIM_FUNC(eslt_ErrorCode) esl_updateVerifyRSARIPEMD160_V15(VSECPRIM_P2VAR_PARA(eslt_WorkSpaceRSAver) workSpace, eslt_Length inputSize, VSECPRIM_P2CONST_PARA(eslt_Byte) input)
{
  ...
  {
    if (actOK != ESL_RSA_V15_UPDATE_HASH_FCT(&(workSpace->wsHashUnion.RIPEMD160ws), inputSize, input, (workSpace)->header.watchdog))
    {
...
VSECPRIM_FUNC(eslt_ErrorCode) esl_finalizeHashVerifyRSARIPEMD160_V15(VSECPRIM_P2VAR_PARA(eslt_WorkSpaceRSAver) workSpace, VSECPRIM_P2VAR_PARA(eslt_Byte) messageDigest)
{
  ...
  if (returnValue == ESL_ERC_NO_ERROR)
  {
    ...
    (void)ESL_RSA_V15_FINALIZE_HASH_FCT(&(workSpace->wsHashUnion.RIPEMD160ws), messageDigest, (workSpace)->header.watchdog);
  }
  return returnValue;
}
                                                                  ...
                                                                  VSECPRIM_P2CONST_PARA(eslt_Byte) messageDigest, eslt_Length signatureSize, VSECPRIM_P2CONST_PARA(eslt_Byte) signature)
{
  ...
  else
  {
    ...
    if (ESL_ERC_NO_ERROR == returnValue)
    {
      ...
      {
        i = (eslt_Length)(keyPairModuleSize - ESL_SIZEOF_RIPEMD160_DIGEST - ((eslt_Length) sizeof(eslt_VERIFY_RSA_RIPEMD160_V15_ASN1_DIGESTINFO)));
        ...
        else
        {
          ...
          {
            i = (eslt_Length)(i + sizeof(eslt_VERIFY_RSA_RIPEMD160_V15_ASN1_DIGESTINFO));
