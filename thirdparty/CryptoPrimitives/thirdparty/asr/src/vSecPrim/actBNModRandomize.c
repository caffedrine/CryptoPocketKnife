  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void)) /* PRQA S 3677 */ /* MD_VSECPRIM_CONST_POINTER */
{
  ...
  actRETURNCODE retVal = actEXCEPTION_RANDOM; /* PRQA S 2981 */ /* MD_MSR_RetVal */
  boolean randomNumberIsZero = TRUE;

  for (count = 0u; count < actBNMODRANDOMIZE_RNG_MAX_RETRIES; count++)
  {
    ...
    randomNumberIsZero = actBNIsZero(x, m_length); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_FIXED_SIZE */
    if ((retVal != actOK) || (randomNumberIsZero == FALSE))
    {
      break;
    }
  }

  /* In case also the last random number was zero, return an error code. */
  if (count == actBNMODRANDOMIZE_RNG_MAX_RETRIES)
  {
    retVal = actEXCEPTION_RANDOM;
  }
