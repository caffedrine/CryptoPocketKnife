...
VSECPRIM_ROM(VSECPRIM_LOCAL, actU32) pi_lane[] = /* PRQA S 3218 */ /* MD_VSECPRIM_8.9 */
{
  ...
  /*  -------------------------------------------  */
  /*  0,0  <-            0,0  */    0u, /*  UNused  */
  /*  1,0  <-            1,1  */    6u, /* iteration number 1  */
  /*  2,0  <-            2,2  */   12u, /* iteration number 7  */
  /*  3,0  <-            3,3  */   18u, /* iteration number 19 */
  /*  4,0  <-            4,4  */   24u, /* iteration number 13 */
  /*  0,1  <-            3,0  */    3u, /* iteration number 18 */
  /*  1,1  <-            4,1  */    9u, /* iteration number 2  */
  /*  2,1  <-            0,2  */   10u, /* iteration number 23 */
  /*  3,1  <-            1,3  */   16u, /* iteration number 16 */
  /*  4,1  <-            2,4  */   22u, /* iteration number 3  */
  /*  0,2  <-            1,0  */    1u, /* iteration number 24 */
  /*  1,2  <-            2,1  */    7u, /* iteration number 22 */
  /*  2,2  <-            3,2  */   13u, /* iteration number 8  */
  /*  3,2  <-            4,3  */   19u, /* iteration number 9  */
  /*  4,2  <-            0,4  */   20u, /* iteration number 5  */
  /*  0,3  <-            4,0  */    4u, /* iteration number 12 */
  /*  1,3  <-            0,1  */    5u, /* iteration number 17 */
  /*  2,3  <-            1,2  */   11u, /* iteration number 21 */
  /*  3,3  <-            2,3  */   17u, /* iteration number 20 */
  /*  4,3  <-            3,4  */   23u, /* iteration number 10 */
  /*  0,4  <-            2,0  */    2u, /* iteration number 6  */
  /*  1,4  <-            3,1  */    8u, /* iteration number 15 */
  /*  2,4  <-            4,2  */   14u, /* iteration number 4  */
  /*  3,4  <-            0,3  */   15u, /* iteration number 11 */
  /*  4,4  <-            1,4  */   21u  /* iteration number 14 */
};
  ...
  VSECPRIM_P2VAR_PARA(actU32) lane10)
{
  ...
  dst = 1u;
  src = pi_lane[1];
  while (src != 1u) /* FETA_VSECPRIM_ACTKECCACKF_WHILE_LOOP */
  {
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  for (r = 0u; r < KECCAK_ROUNDS; r++)
  {
     ...
     * (1) - calculate slice parity      */
      ...
      for (x = 0u; x <= 8u; x += WORD_NUMBER_64BITWORD_UINT32) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
      {
      ...
      for (x = 0u; x <= 8u; x += WORD_NUMBER_64BITWORD_UINT32) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
      {
        ...
        for (y = 0u; y <= (4u * SLICE_LENGTH_UINT32); y += SLICE_LENGTH_UINT32) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
        {
          /* info->state [y + x] ^= Dx; */
      ...
      for (y = 0u; y <= (4u *SLICE_LENGTH_UINT32); y += SLICE_LENGTH_UINT32) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
      {
        for (x = 0u; x <= 8u; x += WORD_NUMBER_64BITWORD_UINT32)/* for (x = 0; x < 5; x++) */ /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
        {
          /* C [x] = info->state [y + x]; */
