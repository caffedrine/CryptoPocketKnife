...
# define ESL_RSA_V15_INIT_HASH_FCT(workspace)                         (void)actRMD160Init(workspace)
...
# define ESL_RSA_V15_FINALIZE_HASH_FCT(workspace, messageDigest, wd)  (void)actRMD160Finalize(workspace, (VSECPRIM_P2VAR_PARA(actU8))(messageDigest), wd)
                                                                  ...
                                                                  VSECPRIM_P2VAR_PARA(eslt_Length) signatureSize, VSECPRIM_P2VAR_PARA(eslt_Byte) signature)
{
  ...
  else
  {
    ...
    else
    {
      ...
      i = (eslt_Length)(i + sizeof(eslt_SIGN_RSA_CRT_RIPEMD160_V15_ASN1_DIGESTINFO));
      ESL_RSA_V15_FINALIZE_HASH_FCT(&(workSpace->wsHashUnion.RIPEMD160ws), &messageBuf[i], (workSpace)->header.watchdog);
