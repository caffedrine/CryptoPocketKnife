 ...
 *                 tgt must provide at least actFIPS186_BASE_LENGTH many bytes.
 *  \context       TASK
 *  \reentrant     TRUE
 *  \synchronous   TRUE
 *********************************************************************************************************************/
VSECPRIM_LOCAL_INLINE_FUNC(actU8) actAddBE(
  VSECPRIM_P2VAR_PARA(actU8) tgt, VSECPRIM_P2CONST_PARA(actU8) src, const actU8 carry);
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  {
    /* actSHAUpdate only returns an error code, if the total input length exceeds uint64max.
     * This cannot happen in a single call. */
    if (actOK == actSHAUpdate(sha1, (VSECPRIM_P2CONST_PARA(actU8)) X, actHASH_BLOCK_SIZE_SHA, watchdog)) /* SBSW_VSECPRIM_CALL_FUNCTION */
    {
      ...
      (void)actAddBE(X_KEY, (VSECPRIM_P2CONST_PARA(actU8))X, 1u); /* SBSW_VSECPRIM_CALL_FUNCTION */

      /* one block of random available - always */
 ...
 *
 */
VSECPRIM_LOCAL_INLINE_FUNC(actU8) actAddBE(VSECPRIM_P2VAR_PARA(actU8) tgt, VSECPRIM_P2CONST_PARA(actU8) src, const actU8 carry)
{
  actU8 cy = carry;
  actU16 tmp;
  actU32 i;
  ...
  {
    tmpSrc = &src[(actFIPS186_BASE_LENGTH - 1u) - i];
    tmpTgt = &tgt[(actFIPS186_BASE_LENGTH - 1u) - i];
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  while (i < length) /* FETA_VSECPRIM_UPPER_BOUNDARY_LARGER_UINT32_MAX */
  {
    ...
    {
      /* pseudoRandom always sets info->bytes_remain = actFIPS186_BASE_LENGTH > 0,
       * so that in the next iteration of the while loop, the if-branch will be entered
       * again and i will be incremented.
       * Thus, the loop will terminate after a finite amount of time. */
