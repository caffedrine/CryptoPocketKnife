...
# define ESL_RSA_V15_INIT_HASH_FCT(workspace)                         (void)actSHA256Init(workspace)
/* Hash update function */
# define ESL_RSA_V15_UPDATE_HASH_FCT(workspace, inputSize, input, wd) actSHA256Update(workspace, (VSECPRIM_P2CONST_PARA(actU8))(input), (actLengthType)(inputSize), wd)
/* Hash finalization function */
# define ESL_RSA_V15_FINALIZE_HASH_FCT(workspace, messageDigest, wd)  (void)actSHA256Finalize(workspace, (VSECPRIM_P2VAR_PARA(actU8))(messageDigest), wd)
...
VSECPRIM_ROM(VSECPRIM_LOCAL, eslt_Byte) eslt_SIGN_RSA_CRT_SHA256_V15_ASN1_DIGESTINFO[] = ASN1_DIGESTINFO_SHA256; /* PRQA S 3218 */ /* MD_VSECPRIM_8.9 */
...
VSECPRIM_FUNC(eslt_ErrorCode) esl_updateSignRSACRTSHA256_V15(VSECPRIM_P2VAR_PARA(eslt_WorkSpaceRSACRTsig) workSpace, const eslt_Length inputSize, VSECPRIM_P2CONST_PARA(eslt_Byte) input)
{
  ...
  else
  {
    ...
    {
      if (actOK != ESL_RSA_V15_UPDATE_HASH_FCT(&(workSpace->wsHashUnion.SHA256ws), inputSize, input, (workSpace)->header.watchdog))
      {
                                                               ...
                                                               VSECPRIM_P2VAR_PARA(eslt_Length) signatureSize, VSECPRIM_P2VAR_PARA(eslt_Byte) signature)
{
  ...
  else
  {
    ...
    {
      /* This check needs to be repeated in esl_finalizeSignRSACRTSHA256_V15, since
       * the check in esl_initSignRSACRTSHA256_V15 only compares
    ...
    else
    {
      ...
      /* Since keyPairModuleSize >= ASN1_MINIMAL_SIGVER_KEY_LENGTH_SHA256 = 62, no overflow occurs in
       * keyPairModuleSize - ASN1_SIZEOF_SHA256_PARAMETERS_WITH_FIXED_LENGTH >=  62 - 54.
      ...
      i = (eslt_Length)(i + padBytesNeeded);
      messageBuf[i] = 0x00u;
      i++;
      actMemCpyByteArray((VSECPRIM_P2VAR_PARA(actU8)) (&messageBuf[i]), eslt_SIGN_RSA_CRT_SHA256_V15_ASN1_DIGESTINFO, sizeof(eslt_SIGN_RSA_CRT_SHA256_V15_ASN1_DIGESTINFO)); /* PRQA S 0315 */ /* MD_VSECPRIM_P2VOID_CAST */
      i = (eslt_Length)(i + sizeof(eslt_SIGN_RSA_CRT_SHA256_V15_ASN1_DIGESTINFO));
      ESL_RSA_V15_FINALIZE_HASH_FCT(&(workSpace->wsHashUnion.SHA256ws), &messageBuf[i], (workSpace)->header.watchdog);
...
/**********************************************************************************************************************
 *  END OF FILE: ESLib_RSA_V15_SigCRT_SHA256.c
 *********************************************************************************************************************/
