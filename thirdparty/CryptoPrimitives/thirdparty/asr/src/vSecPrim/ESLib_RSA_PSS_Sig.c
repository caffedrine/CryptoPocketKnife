  ...
  eslt_HashAlgorithm hashID)
{
  ...
  {
    actReturnValue = actHashUpdate(&(workSpace->wsHashUnion), input, inputSize, hashID, workSpace->header.watchdog); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
    ...
    else
    {
      ...
      if (returnValue == ESL_ERC_NO_ERROR)
      {
        ...
        if (returnValue == ESL_ERC_NO_ERROR)
        {
          ...
          returnValue = esl_calcSaltedHashRSA_PSS(&(workSpace->wsHashUnion), saltLength, dbSalt,
            messageDigest, hashDigestSize, saltedHash, hashID, workSpace->header.watchdog); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
        }
      }
      ...
      if (returnValue == ESL_ERC_NO_ERROR)
      {
        ...
        returnValue = esl_generateMaskMGF1(hashID, &(workSpace->wsHashUnion),
          ...
          workSpace->encodedMessageBuffer.encodedMessage, workSpace->header.watchdog); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
      }
    }
  }
