  ...
  const eslt_Byte lengthFieldSize)
{
  ...
  if (retVal == ESL_ERC_NO_ERROR)
  {
    /* Since esl_initAESCCMLengthFieldCheck did not return an error, it
     * is known that 2 <= lengthFieldSize <= 8, thus no underflow occurs. */
     eslt_Length expectedNonceLength = (eslt_Length)(15u - (eslt_Length)lengthFieldSize);
  ...
  VSECPRIM_P2VAR_PARA(eslt_Length) outSize)
{
  ...
  else
  {
    ...
    {
      *outSize = (eslt_Length)(msgSize - (msgSize % ESL_SIZEOF_AES_BLOCK)); /* SBSW_VSECPRIM_WRITE_POINTER */
    }
  }
  ...
  VSECPRIM_P2VAR_PARA(eslt_Length) outSize)
{
  ...
  else
  {
    ...
    {
      *outSize = (eslt_Length)(encMsgSize - (encMsgSize % ESL_SIZEOF_AES_BLOCK)); /* SBSW_VSECPRIM_WRITE_POINTER */
    }
  }
