  ...
  VSECPRIM_P2VAR_PARA(actBNDIGIT) Nval)
{
  ...
  if (p_length >= n_length)
  {
    ...
    while (actBNCompare(x, Nval, p_length, FALSE) >= actCOMPARE_EQUAL) /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_FIXED_SIZE */ /* FETA_VSECPRIM_ECDSA_REDUCTION_OF_POINT_COORDINATE_BY_GROUP_ORDER */
    {
  ...
  actRETURNCODE retVal = actOK;
   ...
   if (retVal == actOK)
   {
     ...
     if (retVal == actOK)
     {
       ...
       if (actBNCompare(x1, r, n_length, FALSE) != actCOMPARE_EQUAL) /* SBSW_VSECPRIM_FCT_CALL_P2CONST_PARAM */
       {
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  actRETURNCODE retVal = actOK;
   ...
   while (found == FALSE) /* FETA_VSECPRIM_ECDSA_SIGNATURE_GENERATION */
   {
