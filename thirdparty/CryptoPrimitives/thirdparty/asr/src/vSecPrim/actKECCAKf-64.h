  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  for (r = 0u; r < KECCAK_ROUNDS; r++)
  {
    ...
    for (x = 0u; x < SLICE_LENGTH; x++)
    {
      ...
      /* Upper boundary is calculated by values which are known at compile time */
      for (y = 0u; y <= (SLICE_LENGTH * (SLICE_LENGTH - 1u)); y += SLICE_LENGTH) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE */
      {
