  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  if ((info->length + (length % 256u)) < info->length)
  {
    result = actEXCEPTION_LENGTH;
  }
  else
  {
    ...
    if (info->buffer_used > 0u)
    {
      ...
      info->buffer_used = (actU8)(info->buffer_used + diff);  /* SBSW_VSECPRIM_PTR_WRITE_ACCESS_PASSED_BUFFER */
    ...
    if (result != actOK)
    {
      ...
      for (bcount = diff; bcount < lastBlockEnd; bcount += actSIPHASH_BLOCK_SIZE) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE_PARAMETER */
      {
        /* Process block */
