   ...
   VSECPRIM_P2VAR_PARA(actBIGINT) c, actLengthType length)
{
  ...
  else
  {
    ...
    if (actBNCompare(a->value, b->value, length, FALSE) >= actCOMPARE_EQUAL)
    {
      /* case |a|>=|b| */
   ...
   VSECPRIM_P2CONST_PARA(actBNRING) Field, VSECPRIM_P2VAR_PARA(actBNDIGIT) t[])
{
  ...
  */
  ...
  while(actBNIsZero(u, length) == FALSE)  /* while u>0 */
  {
    ...
    /* u >= v ? */
    if (actBNCompare(u, v, length, FALSE) >= actCOMPARE_EQUAL)
    {
  ...
  while(actBNCompare(D.value, x.value, save_length, FALSE) >= actCOMPARE_EQUAL)
  {
