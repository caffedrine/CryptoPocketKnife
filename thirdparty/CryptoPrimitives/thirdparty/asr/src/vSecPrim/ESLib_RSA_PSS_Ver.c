 ...
 *********************************************************************************************************************/
typedef actWorkSpaceUnionHashFunctions_RSA actWorkSpaceUnionHashFunctions;
  ...
  eslt_Length estimatedPSLength)
{
  ...
  if (*saltSize == (eslt_Length)-1) /* PRQA S 2895 */ /* MD_MSR_Dir1.1 */
  {
    ...
    *saltSize = (eslt_Length)((dbLength - 1u) - estimatedPSLength); /* SBSW_VSECPRIM_WRITE_POINTER */
  }
  ...
  eslt_HashAlgorithm hashID)
{
  ...
  {
    /* 9.1.2, Step 2 */
    actReturnValue = actHashUpdate(&(workSpace->wsHashUnion), input, inputSize, hashID, workSpace->header.watchdog); /* SBSW_VSECPRIM_CALL_FUNCTION */
...
{
  /* 9.1.2, Step 2 */
  (void)actHashFinalize(&(workSpace->wsHashUnion), messageDigest, hashID, workSpace->header.watchdog); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
}
  ...
  eslt_HashAlgorithm hashID)
{
  ...
  if (returnValue == ESL_ERC_NO_ERROR)
  {
    ...
    else
    {
      ...
      dbLength = (eslt_Length)(emLength - hashDigestSize - (eslt_Length)(1u));
      ...
      returnValue = esl_generateMaskMGF1(hashID, &(workSpace->wsHashUnion), tempHash, hashDigestSize, hashDigestSize,
        saltedHash, dbLength, db, workSpace->header.watchdog); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
      ...
      if (returnValue == ESL_ERC_NO_ERROR)
      {
        ...
        for (i = 0u; i < (dbLength - 1u); i++) /* FETA_VSECPRIM_UPPER_BOUNDARY_CHECKED_BY_CALLEE_PARAMETER */
        {
          if (0x00u != db[i])
          {
            break;
        ...
        if (returnValue == ESL_ERC_NO_ERROR)
        {
          ...
          psLength = (eslt_Length)(dbLength - saltLength - (eslt_Length)(1u));
          ...
          else
          {
            ...
            returnValue = esl_calcSaltedHashRSA_PSS(&(workSpace->wsHashUnion), saltLength, dbSalt,
              messageDigest, hashDigestSize, tempHash, hashID, workSpace->header.watchdog); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER */
          }
        }
      }
