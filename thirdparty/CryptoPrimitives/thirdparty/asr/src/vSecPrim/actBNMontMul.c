  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  const actLengthType n = Ring->m_length;
  ...
  for (i = 0u; i < n; i++) /* FETA_VSECPRIM_BIGNUM_BOUND_LIMITED_BY_PREVIOUSLY_CHECKED_PARAMETER */
  {
    x_i = x[i];
    ...
    for (j = 1u; j < n; j++) /* FETA_VSECPRIM_BIGNUM_BOUND_LIMITED_BY_PREVIOUSLY_CHECKED_PARAMETER */
    {
  ...
  {
    if (actBNCompare(A, m, n, FALSE) >= actCOMPARE_EQUAL) /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_FIXED_SIZE */
    {
      actL2trigger(watchdog); /* PRQA S 1338, 2983, 3112  */ /* MD_MSR_DummyStmt */ /*lint -e438 */ /* SBSW_VSECPRIM_FUNCTION_CALL_WATCHDOG */
      actBNSub(A, m, A, n); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_FIXED_SIZE */
    }
  ...
  VSECPRIM_P2FUNC(VSECPRIM_NONE, void, watchdog)(void))
{
  ...
  actBNCopy(x, A, Ring->m_length); /* SBSW_VSECPRIM_FCT_CALL_PASSED_BUFFER_FIXED_SIZE */
}
